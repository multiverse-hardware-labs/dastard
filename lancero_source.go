...
func (ls *LanceroSource) distributeData() error {
	...
	for ibuf, dev := range ls.active {
		...
		if ncols != dev.ncols || nrows != dev.nrows || math.Abs(float64(lsync)-float64(dev.lsync)) > 1 || framesUsed <= 0 {
			...
			if ls.blockingReadCount > 1 {
				...
				fmt.Println("************ ls.consecutiveDistributeDataErrors", ls.consecutiveDistributeDataErrors)
				fmt.Printf("have ncols %v, nrows %v, lsync %v, framesUsed %v. had ncols %v, nrows %v, lsync %v",
					ncols, nrows, lsync, framesUsed, dev.ncols, dev.nrows, dev.lsync)
				if ls.consecutiveDistributeDataErrors > 5 && false {
